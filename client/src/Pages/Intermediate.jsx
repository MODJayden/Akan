...
const Intermediate = () => {
  ...
  const interMediateLesson =
    lessons?.filter((lesson) => lesson.level === "Intermediate") || [];
  ...
  const checkAnswers = (lessonId) => {
    const lesson = interMediateLesson?.find((l) => l._id === lessonId);
    let correct = 0;
  ...
  return (
    <>
      {interMediateLesson?.length === 0 ? (
        <div className="flex justify-center items-center h-64">
          No Lessons Found...
        </div>
      ) : (
        <div className="max-w-4xl mx-auto p-4 sm:p-6">
          <Link to={"/akan/language/lessons"} className="fixed left-4 top-24 z-50">
          ...
          <div className="text-center mb-8">
            <h1 className="text-3xl font-bold text-amber-800 mb-2">
              ...
              Intermediate Akan Lessons
            </h1>
            <p className="text-lg text-amber-600">
              Build sentences and understand grammar structures
            </p>
          </div>

          <div className="space-y-4">
            {interMediateLesson?.map((lesson) => (
              <div
                key={lesson._id}
